...
class Cleaner:
	...
	def fix_colname(self):
		...
		return self.df
	def dtype_change(thres: int):
		"""
		TODO: turn cols with unique value < thres to integer, otherwise float
		:param thres: threshold to determine turning a variable to int or float
		"""
		self.df = self.df.apply(lambda col: col.astype(float) if col.dropna().nunique() > thres else col.astype(int))
